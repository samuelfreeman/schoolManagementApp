import { signUpFormSchema as formSchema } from "@/archive/validation/SignUpschema";
...
import { signUp } from "@/archive/api/slices/adminthunk";
...
export function ProfileForm() {
  ...
  return (
    <div className="bg-white  min-h-[503px] w-[544px] m-10 lg:m-0 lg:mr-32 rounded-lg">
      <Form {...form}>
        <FormLabel className="text-4xl text-center justify-center flex p-4">
          Sign Up
        </FormLabel>
        {error && <p className="error">{error}</p>}
        <form
          onSubmit={form.handleSubmit(onSubmit)}
          className="space-y-5   flex justify-center items-center flex-col  p-10 "
        >
          <FormField
